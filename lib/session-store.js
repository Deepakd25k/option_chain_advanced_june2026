const { neon } = require("@neondatabase/serverless");

const SNAPSHOT_BUCKET_MS = 30 * 1000;
const HISTORY_WINDOW_MINUTES = 90;
const MAX_RECENT_SNAPSHOTS = 240;
const OPENING_SNAPSHOT_LIMIT = 20;

let schemaPromise = null;

function databaseConfigured() {
  return Boolean(process.env.DATABASE_URL);
}

function getSql() {
  if (!databaseConfigured()) return null;
  return neon(process.env.DATABASE_URL);
}

async function ensureSchema(sql) {
  if (!schemaPromise) {
    schemaPromise = sql.transaction([
      sql`
        CREATE TABLE IF NOT EXISTS market_snapshots (
          id BIGSERIAL PRIMARY KEY,
          session_date DATE NOT NULL,
          instrument_key TEXT NOT NULL,
          expiry_date DATE NOT NULL,
          captured_at TIMESTAMPTZ NOT NULL,
          bucket_at TIMESTAMPTZ NOT NULL,
          source TEXT NOT NULL,
          spot DOUBLE PRECISION NOT NULL DEFAULT 0,
          atm_strike DOUBLE PRECISION NOT NULL DEFAULT 0,
          payload JSONB NOT NULL,
          created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          UNIQUE (session_date, instrument_key, expiry_date, bucket_at)
        )
      `,
      sql`
        CREATE INDEX IF NOT EXISTS market_snapshots_lookup_idx
        ON market_snapshots (session_date, instrument_key, expiry_date, captured_at)
      `,
      sql`
        CREATE TABLE IF NOT EXISTS session_playbooks (
          id BIGSERIAL PRIMARY KEY,
          session_date DATE NOT NULL,
          instrument_key TEXT NOT NULL,
          expiry_date DATE NOT NULL,
          formula_version TEXT NOT NULL,
          fingerprint JSONB NOT NULL,
          playbook JSONB NOT NULL,
          generated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          UNIQUE (session_date, instrument_key, expiry_date, formula_version)
        )
      `,
      sql`
        CREATE INDEX IF NOT EXISTS session_playbooks_lookup_idx
        ON session_playbooks (instrument_key, session_date DESC)
      `
    ]).catch((error) => {
      schemaPromise = null;
      throw error;
    });
  }
  return schemaPromise;
}

async function saveMarketSnapshot(payload) {
  if (!databaseConfigured()) {
    return { configured: false, saved: false, reason: "DATABASE_URL missing" };
  }
  if (!payload || payload.source !== "live" || !Array.isArray(payload.data) || !payload.data.length) {
    return { configured: true, saved: false, reason: "Live market payload required" };
  }

  const capturedAt = new Date(payload.generatedAt || Date.now());
  if (!isMarketSession(capturedAt)) {
    return { configured: true, saved: false, reason: "Outside NSE/BSE cash session" };
  }

  const sql = getSql();
  await ensureSchema(sql);
  const sessionDate = istDate(capturedAt);
  const bucketAt = new Date(Math.floor(capturedAt.getTime() / SNAPSHOT_BUCKET_MS) * SNAPSHOT_BUCKET_MS);
  const compactPayload = compactSnapshotPayload(payload);
  const spot = number(compactPayload.underlying && compactPayload.underlying.spot);
  const atmStrike = closestStrike(compactPayload.data, spot);

  const rows = await sql`
    INSERT INTO market_snapshots (
      session_date, instrument_key, expiry_date, captured_at, bucket_at,
      source, spot, atm_strike, payload
    ) VALUES (
      ${sessionDate}, ${payload.instrumentKey}, ${payload.expiry}, ${capturedAt.toISOString()},
      ${bucketAt.toISOString()}, ${payload.source}, ${spot}, ${atmStrike}, ${JSON.stringify(compactPayload)}::jsonb
    )
    ON CONFLICT (session_date, instrument_key, expiry_date, bucket_at)
    DO UPDATE SET
      captured_at = EXCLUDED.captured_at,
      spot = EXCLUDED.spot,
      atm_strike = EXCLUDED.atm_strike,
      payload = EXCLUDED.payload
    RETURNING captured_at, (xmax = 0) AS inserted
  `;

  return {
    configured: true,
    saved: true,
    inserted: Boolean(rows[0] && rows[0].inserted),
    capturedAt: rows[0] ? rows[0].captured_at : capturedAt.toISOString(),
    sessionDate
  };
}

async function loadSessionHistory(instrumentKey, expiryDate, at = new Date()) {
  if (!databaseConfigured()) {
    return emptyHistory("DATABASE_URL missing");
  }

  const sql = getSql();
  await ensureSchema(sql);
  const sessionDate = istDate(at);
  const [firstRows, recentRows, statsRows] = await sql.transaction([
    sql`
      SELECT captured_at, payload
      FROM market_snapshots
      WHERE session_date = ${sessionDate}
        AND instrument_key = ${instrumentKey}
        AND expiry_date = ${expiryDate}
      ORDER BY captured_at ASC
      LIMIT ${OPENING_SNAPSHOT_LIMIT}
    `,
    sql`
      SELECT captured_at, payload
      FROM market_snapshots
      WHERE session_date = ${sessionDate}
        AND instrument_key = ${instrumentKey}
        AND expiry_date = ${expiryDate}
      ORDER BY captured_at DESC
      LIMIT ${MAX_RECENT_SNAPSHOTS}
    `,
    sql`
      SELECT COUNT(*)::int AS snapshot_count,
             MIN(captured_at) AS first_saved_at,
             MAX(captured_at) AS last_saved_at
      FROM market_snapshots
      WHERE session_date = ${sessionDate}
        AND instrument_key = ${instrumentKey}
        AND expiry_date = ${expiryDate}
    `
  ], { readOnly: true });

  const candleRows = await loadSessionCandles(sql, sessionDate, instrumentKey, expiryDate);

  const snapshots = dedupePayloads([
    ...firstRows,
    ...recentRows.reverse()
  ]);
  const stats = statsRows[0] || {};
  return {
    configured: true,
    sessionDate,
    snapshots,
    candles5m: candleRows.map((row) => [
      new Date(row.candle_at).toISOString(),
      number(row.open),
      number(row.high),
      number(row.low),
      number(row.close)
    ]),
    snapshotCount: number(stats.snapshot_count),
    firstSavedAt: stats.first_saved_at || null,
    lastSavedAt: stats.last_saved_at || null,
    historyWindowMinutes: HISTORY_WINDOW_MINUTES
  };
}

async function loadSessionCandles(sql, sessionDate, instrumentKey, expiryDate) {
  try {
    return await sql`
      SELECT date_bin(
               INTERVAL '5 minutes',
               captured_at,
               TIMESTAMPTZ '2001-01-01 00:00:00+00'
             ) AS candle_at,
             (array_agg(spot ORDER BY captured_at ASC))[1] AS open,
             MAX(spot) AS high,
             MIN(spot) AS low,
             (array_agg(spot ORDER BY captured_at DESC))[1] AS close
      FROM market_snapshots
      WHERE session_date = ${sessionDate}
        AND instrument_key = ${instrumentKey}
        AND expiry_date = ${expiryDate}
      GROUP BY 1
      ORDER BY 1 ASC
    `;
  } catch (error) {
    console.error("Session candle reconstruction failed", error);
    return [];
  }
}

async function loadLatestCompletedSession(instrumentKey, at = new Date()) {
  if (!databaseConfigured()) return null;
  const sql = getSql();
  await ensureSchema(sql);
  const cutoff = completedSessionCutoff(at);
  const sessions = await sql`
    SELECT session_date, expiry_date, COUNT(*)::int AS snapshot_count
    FROM market_snapshots
    WHERE instrument_key = ${instrumentKey}
      AND (
        session_date < ${cutoff.date}
        OR (${cutoff.includeDate} AND session_date = ${cutoff.date})
      )
    GROUP BY session_date, expiry_date
    ORDER BY session_date DESC, COUNT(*) DESC, expiry_date ASC
    LIMIT 1
  `;
  if (!sessions.length) return null;

  const sessionDate = dateOnly(sessions[0].session_date);
  const expiryDate = dateOnly(sessions[0].expiry_date);
  const [snapshotRows, candleRows, statsRows] = await sql.transaction([
    sql`
      WITH ranked AS (
        SELECT captured_at,
               payload,
               ROW_NUMBER() OVER (
                 PARTITION BY date_bin(
                   INTERVAL '5 minutes',
                   captured_at,
                   TIMESTAMPTZ '2001-01-01 00:00:00+00'
                 )
                 ORDER BY captured_at DESC
               ) AS snapshot_rank
        FROM market_snapshots
        WHERE session_date = ${sessionDate}
          AND instrument_key = ${instrumentKey}
          AND expiry_date = ${expiryDate}
      )
      SELECT captured_at, payload
      FROM ranked
      WHERE snapshot_rank = 1
      ORDER BY captured_at ASC
    `,
    sql`
      SELECT date_bin(
               INTERVAL '5 minutes',
               captured_at,
               TIMESTAMPTZ '2001-01-01 00:00:00+00'
             ) AS candle_at,
             (array_agg(spot ORDER BY captured_at ASC))[1] AS open,
             MAX(spot) AS high,
             MIN(spot) AS low,
             (array_agg(spot ORDER BY captured_at DESC))[1] AS close
      FROM market_snapshots
      WHERE session_date = ${sessionDate}
        AND instrument_key = ${instrumentKey}
        AND expiry_date = ${expiryDate}
      GROUP BY 1
      ORDER BY 1 ASC
    `,
    sql`
      SELECT COUNT(*)::int AS snapshot_count,
             COUNT(DISTINCT date_bin(
               INTERVAL '5 minutes',
               captured_at,
               TIMESTAMPTZ '2001-01-01 00:00:00+00'
             ))::int AS completed_buckets,
             MIN(captured_at) AS first_saved_at,
             MAX(captured_at) AS last_saved_at
      FROM market_snapshots
      WHERE session_date = ${sessionDate}
        AND instrument_key = ${instrumentKey}
        AND expiry_date = ${expiryDate}
    `
  ], { readOnly: true });
  const stats = statsRows[0] || {};
  return {
    sessionDate,
    expiryDate,
    instrumentKey,
    snapshots: dedupePayloads(snapshotRows),
    candles5m: candleRows.map((row) => ({
      start: new Date(row.candle_at).toISOString(),
      open: number(row.open),
      high: number(row.high),
      low: number(row.low),
      close: number(row.close)
    })),
    snapshotCount: number(stats.snapshot_count),
    completedBuckets: number(stats.completed_buckets),
    firstSavedAt: stats.first_saved_at || null,
    lastSavedAt: stats.last_saved_at || null
  };
}

async function loadRecentCompletedSessions(instrumentKey, limit = 5, at = new Date()) {
  if (!databaseConfigured()) return [];
  const sql = getSql();
  await ensureSchema(sql);
  const cutoff = completedSessionCutoff(at);
  const snapshotRows = await sql`
    WITH session_counts AS (
      SELECT session_date,
             expiry_date,
             COUNT(*)::int AS snapshot_count,
             ROW_NUMBER() OVER (
               PARTITION BY session_date
               ORDER BY COUNT(*) DESC, expiry_date ASC
             ) AS expiry_rank
      FROM market_snapshots
      WHERE instrument_key = ${instrumentKey}
        AND (
          session_date < ${cutoff.date}
          OR (${cutoff.includeDate} AND session_date = ${cutoff.date})
        )
      GROUP BY session_date, expiry_date
    ), selected_sessions AS (
      SELECT session_date, expiry_date
      FROM session_counts
      WHERE expiry_rank = 1
      ORDER BY session_date DESC
      LIMIT ${limit}
    ), ranked AS (
      SELECT m.session_date,
             m.expiry_date,
             m.captured_at,
             m.payload,
             ROW_NUMBER() OVER (
               PARTITION BY m.session_date, date_bin(
                 INTERVAL '5 minutes',
                 m.captured_at,
                 TIMESTAMPTZ '2001-01-01 00:00:00+00'
               )
               ORDER BY m.captured_at DESC
             ) AS snapshot_rank
      FROM market_snapshots m
      JOIN selected_sessions s
        ON s.session_date = m.session_date
       AND s.expiry_date = m.expiry_date
      WHERE m.instrument_key = ${instrumentKey}
    )
    SELECT session_date, expiry_date, captured_at, payload
    FROM ranked
    WHERE snapshot_rank = 1
    ORDER BY session_date DESC, captured_at ASC
  `;
  const candleRows = await sql`
    WITH session_counts AS (
      SELECT session_date,
             expiry_date,
             COUNT(*)::int AS snapshot_count,
             ROW_NUMBER() OVER (
               PARTITION BY session_date
               ORDER BY COUNT(*) DESC, expiry_date ASC
             ) AS expiry_rank
      FROM market_snapshots
      WHERE instrument_key = ${instrumentKey}
        AND (
          session_date < ${cutoff.date}
          OR (${cutoff.includeDate} AND session_date = ${cutoff.date})
        )
      GROUP BY session_date, expiry_date
    ), selected_sessions AS (
      SELECT session_date, expiry_date
      FROM session_counts
      WHERE expiry_rank = 1
      ORDER BY session_date DESC
      LIMIT ${limit}
    )
    SELECT m.session_date,
           m.expiry_date,
           date_bin(
             INTERVAL '5 minutes',
             m.captured_at,
             TIMESTAMPTZ '2001-01-01 00:00:00+00'
           ) AS candle_at,
           (array_agg(m.spot ORDER BY m.captured_at ASC))[1] AS open,
           MAX(m.spot) AS high,
           MIN(m.spot) AS low,
           (array_agg(m.spot ORDER BY m.captured_at DESC))[1] AS close
    FROM market_snapshots m
    JOIN selected_sessions s
      ON s.session_date = m.session_date
     AND s.expiry_date = m.expiry_date
    WHERE m.instrument_key = ${instrumentKey}
    GROUP BY m.session_date, m.expiry_date, 3
    ORDER BY m.session_date DESC, 3 ASC
  `;
  const bySession = new Map();
  snapshotRows.forEach((row) => {
    const sessionDate = dateOnly(row.session_date);
    if (!bySession.has(sessionDate)) {
      bySession.set(sessionDate, {
        sessionDate,
        expiryDate: dateOnly(row.expiry_date),
        instrumentKey,
        snapshots: [],
        candles5m: []
      });
    }
    const payload = typeof row.payload === "string" ? JSON.parse(row.payload) : row.payload;
    if (payload) {
      payload.generatedAt = new Date(row.captured_at || payload.generatedAt).toISOString();
      bySession.get(sessionDate).snapshots.push(payload);
    }
  });
  candleRows.forEach((row) => {
    const session = bySession.get(dateOnly(row.session_date));
    if (!session) return;
    session.candles5m.push({
      start: new Date(row.candle_at).toISOString(),
      open: number(row.open),
      high: number(row.high),
      low: number(row.low),
      close: number(row.close)
    });
  });
  return [...bySession.values()].sort((a, b) => b.sessionDate.localeCompare(a.sessionDate));
}

async function loadLatestExpiryClose(instrumentKey, expiryDate, at = new Date()) {
  if (!databaseConfigured() || !expiryDate) return null;
  const sql = getSql();
  await ensureSchema(sql);
  const cutoff = completedSessionCutoff(at);
  const rows = await sql`
    SELECT session_date, expiry_date, captured_at, payload
    FROM market_snapshots
    WHERE instrument_key = ${instrumentKey}
      AND expiry_date = ${expiryDate}
      AND (
        session_date < ${cutoff.date}
        OR (${cutoff.includeDate} AND session_date = ${cutoff.date})
      )
    ORDER BY captured_at DESC
    LIMIT 1
  `;
  if (!rows.length) return null;
  return {
    sessionDate: dateOnly(rows[0].session_date),
    expiryDate: dateOnly(rows[0].expiry_date),
    capturedAt: rows[0].captured_at,
    payload: typeof rows[0].payload === "string" ? JSON.parse(rows[0].payload) : rows[0].payload
  };
}

async function saveSessionPlaybook(playbook) {
  if (!databaseConfigured()) return { configured: false, saved: false };
  const sql = getSql();
  await ensureSchema(sql);
  await sql`
    INSERT INTO session_playbooks (
      session_date, instrument_key, expiry_date, formula_version, fingerprint, playbook
    ) VALUES (
      ${playbook.sessionDate}, ${playbook.instrumentKey}, ${playbook.expiryDate},
      ${playbook.formulaVersion}, ${JSON.stringify(playbook.fingerprint)}::jsonb,
      ${JSON.stringify(playbook)}::jsonb
    )
    ON CONFLICT (session_date, instrument_key, expiry_date, formula_version)
    DO UPDATE SET
      fingerprint = EXCLUDED.fingerprint,
      playbook = EXCLUDED.playbook,
      generated_at = NOW()
  `;
  return { configured: true, saved: true };
}

async function loadPriorSessionPlaybooks(instrumentKey, beforeDate, limit = 30) {
  if (!databaseConfigured()) return [];
  const sql = getSql();
  await ensureSchema(sql);
  const rows = await sql`
    SELECT DISTINCT ON (session_date) playbook
    FROM session_playbooks
    WHERE instrument_key = ${instrumentKey}
      AND session_date < ${beforeDate}
    ORDER BY session_date DESC, generated_at DESC
    LIMIT ${limit}
  `;
  return rows.map((row) => typeof row.playbook === "string" ? JSON.parse(row.playbook) : row.playbook).filter(Boolean);
}

function compactSnapshotPayload(payload) {
  return {
    source: payload.source,
    generatedAt: payload.generatedAt,
    instrumentKey: payload.instrumentKey,
    expiry: payload.expiry,
    underlying: {
      spot: number(payload.underlying && payload.underlying.spot),
      dayOpen: number(payload.underlying && payload.underlying.dayOpen),
      previousClose: number(payload.underlying && payload.underlying.previousClose)
    },
    future: payload.future ? {
      instrumentKey: payload.future.instrumentKey || payload.future.instrument_key || "",
      symbol: payload.future.symbol || "",
      expiry: payload.future.expiry || "",
      ltp: number(payload.future.ltp || payload.future.last_price),
      volume: number(payload.future.volume),
      oi: number(payload.future.oi),
      timestamp: payload.future.timestamp || null
    } : null,
    data: payload.data.map((row) => ({
      strike_price: number(row.strike_price || row.strikePrice || row.strike),
      underlying_spot_price: number(row.underlying_spot_price || row.underlyingSpotPrice || row.underlying_spot),
      pcr: number(row.pcr),
      call_options: compactSide(row.call_options || row.callOption || row.ce || row.CE),
      put_options: compactSide(row.put_options || row.putOption || row.pe || row.PE)
    }))
  };
}

function compactSide(side = {}) {
  const market = side.market_data || side.marketData || side.market || side;
  const greeks = side.option_greeks || side.optionGreeks || side.greeks || {};
  return {
    instrument_key: side.instrument_key || side.instrumentKey || "",
    market_data: {
      ltp: number(market.ltp || market.last_price || market.lastPrice),
      volume: number(market.volume),
      oi: number(market.oi || market.open_interest || market.openInterest),
      prev_oi: number(market.prev_oi || market.prevOi || market.previous_oi || market.previousOpenInterest),
      bid_price: number(market.bid_price || market.bidPrice || market.bid),
      ask_price: number(market.ask_price || market.askPrice || market.ask),
      bid_qty: number(market.bid_qty || market.bidQty || market.bid_quantity),
      ask_qty: number(market.ask_qty || market.askQty || market.ask_quantity)
    },
    option_greeks: {
      delta: number(greeks.delta),
      gamma: number(greeks.gamma),
      theta: number(greeks.theta),
      vega: number(greeks.vega),
      iv: number(greeks.iv)
    }
  };
}

function isMarketSession(date) {
  const parts = istParts(date);
  if (parts.weekday === "Sat" || parts.weekday === "Sun") return false;
  const minute = parts.hour * 60 + parts.minute;
  return minute >= 9 * 60 + 15 && minute <= 15 * 60 + 30;
}

function istDate(date) {
  const parts = istParts(date);
  return `${parts.year}-${pad(parts.month)}-${pad(parts.day)}`;
}

function completedSessionCutoff(date) {
  const parts = istParts(date);
  const minute = parts.hour * 60 + parts.minute;
  const weekday = parts.weekday;
  const weekdaySession = weekday !== "Sat" && weekday !== "Sun";
  return {
    date: `${parts.year}-${pad(parts.month)}-${pad(parts.day)}`,
    includeDate: !weekdaySession || minute > 15 * 60 + 30
  };
}

function dateOnly(value) {
  if (typeof value === "string") return value.slice(0, 10);
  return new Date(value).toISOString().slice(0, 10);
}

function istParts(date) {
  const values = new Intl.DateTimeFormat("en-CA", {
    timeZone: "Asia/Kolkata",
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
    hour: "2-digit",
    minute: "2-digit",
    hourCycle: "h23",
    weekday: "short"
  }).formatToParts(date).reduce((result, part) => {
    result[part.type] = part.value;
    return result;
  }, {});
  return {
    year: Number(values.year),
    month: Number(values.month),
    day: Number(values.day),
    hour: Number(values.hour),
    minute: Number(values.minute),
    weekday: values.weekday
  };
}

function dedupePayloads(rows) {
  const byTime = new Map();
  rows.forEach((row) => {
    const payload = typeof row.payload === "string" ? JSON.parse(row.payload) : row.payload;
    if (!payload) return;
    payload.generatedAt = new Date(row.captured_at || payload.generatedAt).toISOString();
    byTime.set(payload.generatedAt, payload);
  });
  return [...byTime.values()].sort((a, b) => new Date(a.generatedAt) - new Date(b.generatedAt));
}

function closestStrike(rows, spot) {
  return rows.reduce((best, row) => {
    const strike = number(row.strike_price || row.strikePrice || row.strike);
    if (!best || Math.abs(strike - spot) < Math.abs(best - spot)) return strike;
    return best;
  }, 0);
}

function emptyHistory(reason) {
  return {
    configured: false,
    reason,
    snapshots: [],
    candles5m: [],
    snapshotCount: 0,
    firstSavedAt: null,
    lastSavedAt: null,
    historyWindowMinutes: HISTORY_WINDOW_MINUTES
  };
}

function number(value) {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : 0;
}

function pad(value) {
  return String(value).padStart(2, "0");
}

module.exports = {
  databaseConfigured,
  isMarketSession,
  loadLatestCompletedSession,
  loadLatestExpiryClose,
  loadPriorSessionPlaybooks,
  loadRecentCompletedSessions,
  loadSessionHistory,
  saveSessionPlaybook,
  saveMarketSnapshot
};
