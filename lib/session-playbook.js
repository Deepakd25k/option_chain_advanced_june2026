...
function findImmediateOiLevel(rows, spot, side, step) {
  ...
  const candidates = rows.filter((row) => (support
    ...
    .sort((a, b) => b.oi - a.oi || Math.abs(a.strike - spot) - Math.abs(b.strike - spot));
  if (candidates[0]) return { ...candidates[0], selection: "nearest-three-strike-max-oi" };
...
function buildSessionStory(input) {
  ...
  if (input.wallDirection === "up" && input.spotChange > 0) {
    learning = `Both immediate nearest-3-strike OI levels migrated higher with spot: PE ${supportText}, CE ${resistanceText}; spot gained ${signed(input.spotChange)} points.`;
  } else if (input.wallDirection === "down" && input.spotChange < 0) {
    learning = `Both immediate nearest-3-strike OI levels migrated lower with spot: PE ${supportText}, CE ${resistanceText}; spot lost ${signed(input.spotChange)} points.`;
